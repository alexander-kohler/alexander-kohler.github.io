...
input.addEventListener('keyup', (e) => {
  ...
  if (items.length > 0) {
    ...
    for (let i in items) {
      ...
      const matchString = items[i].matches[0].value;
      const matchBegin = items[i].matches[0].indices[0][0];
      const matchEnd = items[i].matches[0].indices[0][1];

      const matchPart1 = matchString.substring(matchBegin-20, matchBegin)
      const matchHighlighted = matchString.substring(matchBegin, matchEnd+1)
      const matchPart2 = matchString.substring(matchEnd+1, matchEnd+20)
      ...
      itemList += `<div class="resultItem">`
        ...
        + `<div>${matchPart1}<span class="resultItem__highlighted">${matchHighlighted}</span>${matchPart2}</div>`
        + `</div>`
        + `</div>`
    }
...
[
  ...
  {
      "item": {
          "content": "jfdklsa;jfkldsa; fdjskal;jk\ngvdfsa ",
          "date": "2024/07/05",
          "image": "http://localhost:1313/posts/my-first-post/image.jpeg",
          "permalink": "http://localhost:1313/posts/my-first-post/",
          "summary": "jfdklsa;jfkldsa; fdjskal;jk\ngvdfsa ",
          "title": "My First Post"
      },
      "refIndex": 1,
      "matches": [
